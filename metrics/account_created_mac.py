...
for x in range(0, len(WEEKS)):
    ...
    for week in WEEKS[x:]:
        ...
        if not saved_uids:
            ...
            signed_events = sqlContext.sql("SELECT C4 FROM " + table_name + " WHERE C5 = 'account.created' AND C3 = 'Macintosh'")
            ...
            created_events = sqlContext.sql("SELECT C4 FROM " + table_name + " WHERE C5 = 'account.signed' AND C3 = 'Macintosh'")
