...
for x in range(0, len(WEEKS)):
    ...
    for week in WEEKS[x:]:
        ...
        if not saved_uids:
            ...
            df2.registerTempTable(table_secondary)
            secondary_events = sqlContext.sql(COHORT_QUERY_SECONDARY % table_secondary)
            secondary_new_uids = secondary_events.map(lambda p: p.C4).distinct()

            multi_device_uids = new_uids.intersection(secondary_new_uids)
            saved_uids = multi_device_uids
            saved_uids_count = int(multi_device_uids.count())
